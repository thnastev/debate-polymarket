/**
 * Tabbycat / Calicotab ingestion (§8).
 *
 * THE BROWSER CANNOT FETCH CALICOTAB. A page served from your domain cannot
 * call calicotab.com unless Calicotab sends permissive CORS headers, which it
 * does not. That is a browser rule, not a Tabbycat one. So there is no fetch
 * in this file: the live path is the `sync-tab` Edge Function, and this is the
 * paste-box parser that is the escape hatch when the API is closed, the
 * network is bad, or the tournament is not on Tabbycat at all.
 *
 * Everything here is a pure parser over pasted JSON, so it is testable and
 * cannot be blamed on a network.
 */
